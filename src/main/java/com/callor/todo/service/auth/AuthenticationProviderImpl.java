...
public class AuthenticationProviderImpl  implements AuthenticationProvider{
	...
	public Authentication authenticate(Authentication authentication) throws AuthenticationException {
		...
		// Token에 UserVO가 담기게 되고
		// Security 에서 제공하는 user 정보 이외의 항목
		// (UserVO에 임의로 설정한 항목들 : realname, nickname, email 등등)
		// 들을 JSP 에서 자유롭게 principal 객체를 통해 접근할수 있다
		 ...
		 * Token을 발행하는 최소한의 요건
		 * 이렇게 Token을 발행하면 JSP에서 principal.username, principal.password 항목은 참조할수 있으나
		 * principal.email, principal.realname 등의 항목은 참조할 수 없음 
		 */
